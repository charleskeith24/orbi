...
export function MonthlyRecommendations({
  ...
  async function generate() {
    if (hasText) {
      const ok = await confirm({
        title: "Replace the recommendations?",
        description: "The new draft replaces the summary and all five lists. Nothing is saved until you click Save.",
  ...
  function save() {
    ...
    const summary = summarizeMonthlyReport(report, db, now)
  ...
  const hint = !hasText
    ? "Draft it from this month's numbers with AI, or write it yourself."
    : canSave
      ? hasMonthlyText(baseline)
        ? "Unsaved changes."
        : "Not saved yet."
      : status === "final"
        ? "Saved as final, with this month's numbers frozen alongside."
        : "Saved as a draft."

  return (
    <SectionCard
      title="Monthly Strategic Recommendations"
      description="Continue · Increase · Reduce · Stop · Experiment — grounded in this month's numbers."
      icon={Compass}
      className={className}
      action={
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
          ...
          {saved ? <span className="num">Saved {formatDateTime(saved.updated_at)}</span> : null}
        </div>
        {engine && engine.provider !== "manual" ? <AiNotice className="print:hidden" /> : null}
        <AiErrorNotice error={ai.error} title="Couldn't draft the recommendations." onRetry={() => void generate()} />
        ...
        <div className="flex min-w-0 flex-col gap-1.5 print:break-inside-avoid">
          ...
          <Textarea
            ...
            placeholder="The month in 3–4 sentences, with the numbers that matter."
