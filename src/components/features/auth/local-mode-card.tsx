    ...
    <section className="rounded-lg border bg-card p-5 text-card-foreground shadow-xs sm:p-6">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 space-y-1">
          <h1 className="text-base font-semibold">Accounts are off in local mode</h1>
          <p className="text-sm text-muted-foreground">
            You&apos;re running in local mode: your workspace lives in this browser.
          </p>
        </div>
      </div>
      <p className="mt-4 text-sm text-muted-foreground">
        To turn on accounts and sync, set these environment variables and restart the app:
      </p>
      <ul
        aria-label="Supabase environment variables"
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Setup guide: <code className="font-mono text-foreground">docs/SUPABASE.md</code>
      </p>
      <Button asChild size="lg" className="mt-5 w-full">
        <Link href="/">
          Open my local workspace
          <ArrowRight data-icon="inline-end" aria-hidden />
        </Link>
      </Button>
      <p className="mt-3 text-center text-xs text-muted-foreground">
        Back up or move your data any time from Settings → Data.
      </p>
    </section>
  )
}
