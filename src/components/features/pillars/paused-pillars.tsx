...
export function PausedPillars({
  ...
  return (
    <SectionCard title={t("paused_title")} count={pillars.length} info={t("paused_section_description")} contentClassName="p-0">
        ...
        {pillars.map((pillar) => (
          <li key={pillar.id} className="flex items-center gap-3 px-4 py-2.5">
            ...
            >
              ...
              {pillar.description ? <span className="block truncate text-xs text-muted-foreground">{pillar.description}</span> : null}
            </button>
