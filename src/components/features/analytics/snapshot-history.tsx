...
const SOURCE_LABEL: Record<MetricSource, `source_${MetricSource}`> = {
              ...
              <tr key={s.id} className="align-top">
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1.5 whitespace-nowrap">
                    {date}
                    {index === 0 ? (
                      <span className="rounded-sm bg-muted px-1 text-[11px] font-medium text-muted-foreground">{t("latest")}</span>
                    ) : null}
                  </div>
                  <p className="max-w-56 truncate text-xs text-muted-foreground" title={s.notes || undefined}>
                    {[SOURCE_LABEL[s.source] ? t(SOURCE_LABEL[s.source]) : s.source, s.notes].filter(Boolean).join(" · ")}
                  </p>
                </td>
                ...
                <td className="px-2 py-1.5 text-right whitespace-nowrap">
                  <Button variant="ghost" size="icon-xs" aria-label={t("edit_from", { date })} onClick={() => onEdit(s)}>
                    <Pencil aria-hidden />
                  </Button>
                  <Button
                    variant="ghost"
