  ...
  return (
    <div className={cn("flex min-w-0 flex-col gap-2 rounded-lg border bg-card p-3", className)}>
      <p className="flex items-center gap-1.5 text-sm font-medium">
        ...
        <span>
          Brand HQ <span className="num">{completeness.pct}%</span> complete
        </span>
      </p>
      <Meter
        ...
        aria-label="Brand HQ completeness"
      />
      <p className="text-xs text-pretty text-muted-foreground">
        {next ? "The AI writes better when this is complete." : "Every AI generation has the full picture of your brand."}
      </p>
      {next ? (
        <button
          type="button"
        ...
        >
          ...
          <span className="truncate font-medium text-foreground group-hover/next:underline">{next.label}</span>
...
export function BrandSectionNav({
  ...
  return (
    <nav aria-label="Brand HQ sections" className="-mx-4 min-w-0 md:-mx-6 lg:mx-0">
            ...
            <li key={section.key} className="shrink-0">
              ...
              >
                ...
                <span className="ml-auto flex shrink-0 items-center gap-1.5 pl-1">
                  ...
                  {complete ? (
                    <CircleCheck className="size-3.5 text-good-fg" aria-label="Complete" />
                  ) : (
