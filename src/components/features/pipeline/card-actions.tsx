            ...
            {PIPELINE_STAGES.map((stage) => {
              ...
              return (
                <DropdownMenuItem key={stage.id} disabled={current} onSelect={() => actions.moveItem(item, stage.id)}>
                  ...
                  {current ? <Check className="text-muted-foreground" aria-label="Current stage" /> : null}
                </DropdownMenuItem>
              )
            })}
          </DropdownMenuSubContent>
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent className="w-60" onPointerDown={(event) => event.stopPropagation()}>
              {dueDateShortcuts(now, settings.week_starts_on).map((shortcut) => (
                <DropdownMenuItem key={shortcut.label} onSelect={() => actions.setDueDate(item, shortcut.value)}>
                  {shortcut.label}
                  <DropdownMenuShortcut className="tracking-normal">{formatDate(shortcut.value, "EEE, MMM d")}</DropdownMenuShortcut>
                </DropdownMenuItem>
              ))}
              <DropdownMenuItem onSelect={() => actions.pickDueDate(item)}>Pick a date…</DropdownMenuItem>
              {item.due_date ? (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onSelect={() => actions.setDueDate(item, null)}>Clear due date</DropdownMenuItem>
                </>
              ) : null}
            </DropdownMenuSubContent>
          </DropdownMenuSub>
