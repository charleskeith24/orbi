...
const VIEW_OPTIONS: ViewOption<StoryView>[] = [
...
export function StoryFilterBar({
  ...
  const pillarOptions = useMemo<FacetOption[]>(() => {
    ...
    if (counts.has(NO_PILLAR)) options.push({ value: NO_PILLAR, label: "No pillar", count: counts.get(NO_PILLAR) ?? 0 })
    return options
  }, [stories, pillars])
  ...
  const usageOptions = useMemo<FacetOption[]>(() => {
    ...
    return [
      { value: "used", label: "Used in ideas", count: used },
      { value: "unused", label: "Not used yet", count: stories.length - used },
    ]
  }, [stories, usage])
      ...
      actions={
        <>
          <OptionSelect
            ...
            className="w-40"
          />
          <ViewToggle value={view} onChange={(next) => onChange({ view: next === "grid" ? "" : next })} options={VIEW_OPTIONS} aria-label="Story view" />
        </>
      }
    >
      <SearchInput value={filters.q} onChange={(q) => onChange({ q })} placeholder="Search stories…" />
      ...
      <FacetFilter title="Usage" options={usageOptions} value={filters.usage} onChange={(value) => onChange({ usage: value.join(",") })} />
      <Button
        type="button"
        variant="outline"
        size="sm"
