...
const PLACEHOLDER = "Today I had to talk to my team because deadlines were being missed."
const EXAMPLE =
...
export function ExperienceView() {
  ...
  function onStorySaved(story: Story) {
    ...
    toast.success("Saved to your Story Vault", {
      description: relink.length ? `${story.title} · linked to the ${pluralize(relink.length, "idea")} you saved` : story.title,
      action: { label: "Open", onClick: () => router.push(`/stories?open=${story.id}`) },
    })
  }
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Describe something that really happened. Get a Story Vault entry and eight content angles — hooks, outlines and drafts in your voice."
        actions={
      ...
      <div className="grid min-w-0 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <SectionCard
          ...
          description="A few honest sentences are enough — what happened, what you did, how it turned out and what you learned."
        >
          <form
            onSubmit={(event) => {
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-xs text-muted-foreground">
              <span>
                {length > 0 && !ready
                  ? `Keep going — at least ${MIN_EXPERIENCE_CHARS} characters.`
                  : "Real details beat polish: numbers, names of things, how it felt. ⌘/Ctrl + Enter generates."}
              </span>
              <span className="num">
            ...
            <div className="flex flex-wrap items-end gap-3">
              <FormField label="Content Pillar (optional)" htmlFor={`${id}-pillar`} className="w-full sm:w-64">
                <PillarSelect
                  id={`${id}-pillar`}
                  size="sm"
                  allowNone
                  noneLabel="Let the AI choose"
                  placeholder="Let the AI choose"
                  value={page.pillarId}
      ...
      {hasResults ? (
        <div className="grid min-w-0 items-start gap-4 xl:grid-cols-[20rem_minmax(0,1fr)]">
          ...
          <section aria-label="Content angles" className="min-w-0">
            <AngleResults
              ...
              columns={2}
              notice="Every angle comes from what you wrote. Edit the drafts until they sound like you, then save the ones you would actually post."
            />
          </section>
        </div>
      ) : (
        <EmptyState
          icon={Sparkles}
          title="One real moment, eight pieces of content"
          description="Leadership lesson, management framework, personal reflection, storytelling post, educational video, contrarian opinion, LinkedIn post and Facebook post — each with a hook, an outline and a draft, plus a Story Vault entry you can keep."
          action={
            text ? undefined : (
