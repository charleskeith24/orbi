...
/** Shallow URL update — Next mirrors `history.replaceState` into `useSearchParams` (no server round trip). */
...
/**
 * Page state mirrored to the URL (`?open=`, `?q=`, filters, view). Local state is the source of
 * truth so typing never waits on navigation; changes are written with `history.replaceState`.
 * A URL change this hook didn't write (a ⌘K result, a cross-link to the same page) replaces the
 * state. `keys` must be a module-level constant.
 */
