...
import { CalendarCheck2, CalendarDays, CalendarRange, ChevronDown, ChevronLeft, ChevronRight, ListFilter, Target } from "lucide-react"
import { useMemo, useState } from "react"
import { FacetFilter, ResetFiltersButton, ViewToggle, type FacetOption, type ViewOption } from "@/components/common"
...
/** Today · previous / next · the period label (click to jump to a date). */
...
/** Posting targets on/off and Month · Week · Day (the page header's actions). */
export function CalendarViewControls({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <Target aria-hidden />
        <span className="hidden lg:inline">{t("posting_targets")}</span>
        <span className="sr-only lg:hidden">{t("posting_targets")}</span>
      </Button>
...
/** The five facets (on phones they fold behind `FiltersToggle`). */
export function CalendarFilterBar({
  ...
  return (
    <div id={id} className={cn("flex min-w-0 flex-wrap items-center gap-2", className)}>
...
/** Phones: the "Filters" button that folds the facets away, with how many are in use. */
export function FiltersToggle({
  ...
  const t = useT(calendarMessages)
  const inUse = (Object.keys(filters) as FilterKey[]).filter((key) => filters[key].length > 0).length
  return (
    <Button type="button" variant="outline" size="sm" aria-expanded={open} aria-controls={controls} onClick={() => onOpenChange(!open)}>
