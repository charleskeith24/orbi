...
import { PUBLISHED_STAGES, REPURPOSE_TYPES } from "@/lib/constants"
...
/** Title (click to edit), provenance line (source, collab, last edit), the next pipeline action and the property chips. */
      ...
      <div className="flex flex-wrap items-start gap-x-4 gap-y-3">
        <div className="flex min-w-0 flex-1 basis-80 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            />
            {/* Platform and format are the chips below; this line is where it came from and when it changed. */}
            <p className="mt-0.5 flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-muted-foreground">
              {parent ? (
                <>
                  {/* "Source" links to the piece it was repurposed from; the full "Carousel of “…”" is its tooltip. */}
                  <Link
              ...
              {updated ? (
                <>
                  {parent ? <span aria-hidden>·</span> : null}
