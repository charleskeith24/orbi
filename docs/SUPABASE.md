...
- Row-level security on every table. Workspace tables have `select` / `insert` / `update` / `delete` policies for `authenticated` using `(select auth.uid()) = user_id`; `public.users` allows reading and updating only your own row (updates: the profile columns only, never `email` or `id`); other people's profiles come only from `get_profiles()`, which never returns the email. `anon` has no table access (Supabase grants it by default; the migrations revoke it).
